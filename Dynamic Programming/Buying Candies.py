...
# Similar to 0/N knapsack, here value is A[i]*B[i], weight is C[i] and capacity is D
...
class Solution:
    ...
    # @param D : integer
    # @return an integer
    def solve(self, A, B, cost, total_money):
        n = len(A)
            
        dp = [[0 for j in range(total_money + 1)] for i in range(n + 1)]
        
        for i in range(1, n+1):
            for j in range(1, total_money+1):
                if cost[i-1] <= j:
                    dp[i][j] = max(dp[i-1][j], A[i-1]*B[i-1] + dp[i][j-cost[i-1]])
